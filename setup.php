...
function plugin_init_notifier(): void
{
    ...
    $PLUGIN_HOOKS['config_page']['notifier'] = 'front/config.form.php';

    // GLPI 11 prefers public/, GLPI 10 uses css/ and js/. public/ is
    // generated by tools/sync-assets.sh and CI fails on drift.
    ...
    // On for everyone by default. Only helpdesk sessions pay for the lookup.
    if (
        isset($_SESSION['glpiactiveprofile']['interface'])
        && $_SESSION['glpiactiveprofile']['interface'] === 'helpdesk'
        && !NotifierConfig::get('helpdesk_enabled')
    ) {
    ...
    $dispatch = [Notification::class, 'handleItemEvent'];
    ...
    // item_purge → the item is gone for good, drop the rows.
    $cleanup = [Notification::class, 'cleanForItem'];
    foreach ($watched_types as $type) {
        $PLUGIN_HOOKS[Hooks::ITEM_PURGE]['notifier'][$type] = $cleanup;
    }

    // Binned items may come back: stop nagging, keep the history.
    $silence = [Notification::class, 'silenceForItem'];
    foreach (['Ticket', 'Change', 'Problem', 'ProjectTask'] as $type) {
        $PLUGIN_HOOKS[Hooks::ITEM_DELETE]['notifier'][$type] = $silence;
    }

    // Any route to the form clears that item's bells for the viewer.
    $PLUGIN_HOOKS[Hooks::PRE_ITEM_FORM]['notifier'] =
        [Notification::class, 'markItemAsSeen'];
}
